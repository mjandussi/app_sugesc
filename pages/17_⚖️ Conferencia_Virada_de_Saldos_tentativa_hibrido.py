...
opcao_origem = st.radio(
    ...
    ("SIAFERIO (Imprimir Balancetes)", "FLEXVISION (Consultas 079124 e 079125)", "Híbrido"),
    horizontal=True
)
...
def processar_virada_exercicio(arq_ant, arq_prox, arq_pc, origem):
    ...
    elif "FLEXVISION" in origem:
        df_ant = pd.read_excel(arq_ant, header=3).iloc[:-7]
        df_prox = pd.read_excel(arq_prox, header=3).iloc[:-7]
    else:
        df_ant = pd.read_excel(arq_ant, header=3).iloc[:-7] # FLEXVISION
        df_prox = pd.read_excel(arq_prox, header=8).iloc[1:-3] # SIAFERIO
    ...
    if "FLEXVISION" in origem or "Híbrido" in origem:
