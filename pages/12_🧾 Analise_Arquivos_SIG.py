...
def ensure_columns(df: pd.DataFrame, columns):
    """Garantir que colunas existam, preenchendo com zero."""
    for col in columns:
...
def process_txt(uploaded_file):
    ...
    ensure_columns(df, COLS_VAL)

    # Conversão das colunas de valores para float
    for col in COLS_VAL_TXT:
...
cols = st.columns(len(COLS_VAL_TXT))
for idx, col in enumerate(COLS_VAL_TXT):
    cols[idx].metric(
        col.replace('_', ' ').title(),
