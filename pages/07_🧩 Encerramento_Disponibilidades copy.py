...
def combinar_regras(df_regras: pd.DataFrame) -> pd.DataFrame:
    """
    Une as expressões por (ug, ano_fonte, tipo_deta) com 'OU',
    removendo a UG de dentro de cada cláusula e colocando a UG apenas no final.
    Parênteses: ( (cláusula) OU (cláusula) OU ... ) e UG = xxxx
    """
    ...
    grupos = ["ug", "ano_fonte", "tipo_deta"]

    # remove a UG do final da expressão (se houver) e tira espaços/parênteses sobrando na borda
    re_ug = re.compile(
        r"\s*e\s*\[UNIDADE GESTORA EMITENTE\]\.\[CÓDIGO\]\s*=\s*\d+\s*\)?\s*$",
        flags=re.IGNORECASE
    )

    def limpar_clausula(expr: str) -> str:
        e = re_ug.sub("", expr.strip())

        # remove UM par de parênteses externos supérfluos, se houver
        # (sem tentar “descascar” vários níveis)
        if e.startswith("(") and e.endswith(")"):
            e = e[1:-1].strip()
        return e

    def juntar(grupo: pd.DataFrame) -> pd.Series:
        ug = grupo["ug"].iloc[0]
        fontes = ", ".join(sorted(grupo["FONTE_STN"].astype(str).unique()))
        expressoes = [limpar_clausula(x) for x in grupo["expressao"].astype(str).tolist()]

        # (expr_1) OU (expr_2) OU ...
        ou_chain = " OU ".join(f"({e})" for e in expressoes)

        # ( (expr_1) OU (expr_2) OU ... ) e UG=xxxxxx
        expressao_final = f"({ou_chain}) e [UNIDADE GESTORA EMITENTE].[CÓDIGO] = {ug}"

        return pd.Series({
            "FONTE_STN": fontes,
            "quantidade_regras": len(expressoes),
            "expressao_combinada": expressao_final,
        })

    return (
        df_regras.groupby(grupos, dropna=False)
        .apply(juntar)
        .reset_index()
    )
...
def montar_regras_por_ug(df: pd.DataFrame, max_terms_por_expressao: int = 80) -> pd.DataFrame:
    ...
    return (
        pd.DataFrame(regras)
          .sort_values(["ug", "ano_fonte", "tipo_deta", "FONTE_STN", "parte"])
          .reset_index(drop=True)
    )
...
def processar_txt(raw_text: str) -> pd.DataFrame:
    ...
    df_encerr = df_encerr[['ug', 'ano_fonte', 'FONTE_STN', 'marcador_fonte', 'tipo_deta', 'detalhamento']]
...
with tab0:
        ...
        if st.button("🔄 Processar Arquivo", type="primary"):
            with st.spinner("Processando dados..."):
                try:
                    ...
                    # Combinar as regras em expressões únicas
                    df_regras_combinadas = combinar_regras(df_regras)
                    st.session_state['df_regras_combinadas'] = df_regras_combinadas
    ...
    if 'df_encerr' in st.session_state and 'df_regras' in st.session_state:
        ...
        with st.expander("📊 Ver a Tabela de Erros Processada"):
            ...
            with col4:
                fontes = ['Todos'] + sorted(df_encerr['FONTE_STN'].unique().tolist())
            ...
            if fonte_filter != 'Todos':
                df_filtered = df_filtered[df_filtered['FONTE_STN'] == fonte_filter]
        ...
        with col5:
            st.metric("Fontes Únicas", df_regras['FONTE_STN'].nunique())
        ...
        with st.expander("🔍 Ver Detalhes de uma Regra Específica"):
            ...
            if regra_idx < len(df_regras):
                ...
                st.write("**Fonte:**", regra['FONTE_STN'])
        ...
        st.markdown("### 🧩 Regras Agrupadas (parênteses enxutos, UG única no final)")
        dfc = st.session_state['df_regras_combinadas']
        st.dataframe(dfc, use_container_width=True, height=500)

        csv_regras_comb = convert_df_to_csv_com_zfill(dfc)  # sem zfill; só tem ug/ano/tipo/expressão
        st.download_button(
            label="📥 Download CSV (Regras Combinadas)",
            data=csv_regras_comb,
            file_name="df_regras_combinadas.csv",
            mime="text/csv",
            type="primary",
        )
