...
async def analisar_msc_mensal(
    ...
    resultados, df_resumo = run_all_d1(ente, ano, mes_limite, incluir_encerramento)
...
if run:
    ...
    if not ente or len(ente) != 2 or not ente.isdigit():
        st.error("❌ Por favor, informe um código IBGE válido (7 dígitos numéricos)")
        st.stop()
    ...
    try:
        ...
        resultados, df_resumo = run_all_d1(ente, ano, mes, incluir_encerramento)
